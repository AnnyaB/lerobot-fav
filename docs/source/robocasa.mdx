...
RoboCasa exposes two task-string sources for policy conditioning. `--env.task_description_source=lang` (the backward-compatible default) uses RoboCasa's natural-language episode instruction. `--env.task_description_source=task` uses the canonical task name. The released `lerobot/smolvla_robocasa` checkpoint was trained on canonical task names, so use `task` when reproducing that checkpoint's evaluation; otherwise the instruction itself is out of distribution. The released checkpoint examples below also pin `--env.split=target` so instruction-source comparisons use the target benchmark distribution rather than silently mixing evaluation protocols.
...
lerobot-eval \
  ...
  --env.split=target \
  --env.task_description_source=task \
  --eval.batch_size=1 \
  --eval.n_episodes=20 \
